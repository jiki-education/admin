...
jest.mock("marked", () => ({
  marked: {
    parse: jest.fn((markdown: string) =>
      Promise.resolve(`<p>${markdown.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")}</p>`)
    )
  }
}));
...
describe("MarkdownEditor", () => {
  ...
  test("renders in edit mode by default", () => {
    render(<MarkdownEditor value="" onChange={mockOnChange} label="Test Editor" />);
  ...
  test("switches to preview mode and renders markdown", async () => {
    render(<MarkdownEditor value="Hello **world**" onChange={mockOnChange} label="Test Editor" />);
  ...
  test("shows error message when provided", () => {
    render(<MarkdownEditor value="" onChange={mockOnChange} label="Test Editor" error="This field is required" />);

    expect(screen.getByText("This field is required")).toBeInTheDocument();
  });

  test("shows required indicator when required prop is true", () => {
    render(<MarkdownEditor value="" onChange={mockOnChange} label="Test Editor" required />);
