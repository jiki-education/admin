...
describe('Lessons Management E2E Tests', () => {
  ...
  describe('Lesson Creation Flow', () => {
    ...
    it('should display level context information when authenticated', async () => {
      ...
      const currentUrl = page.url();
      if (currentUrl.includes('/signin')) {
        console.log('Redirected to signin - skipping context test');
        return;
      }
    ...
    it('should have lesson type dropdown with all expected options when authenticated', async () => {
      ...
      const currentUrl = page.url();
      if (currentUrl.includes('/signin')) {
        console.log('Redirected to signin - skipping dropdown test');
        return;
      }
      ...
      try {
        ...
        const options = await typeSelect.$$eval('option', options => 
        ...
        console.log('Could not test dropdown - form not accessible');
      }
    });
    ...
    it('should handle JSON data editor when accessible', async () => {
      ...
      const currentUrl = page.url();
      if (currentUrl.includes('/signin')) {
        console.log('Redirected to signin - skipping JSON editor test');
        return;
      }
      
      try {
        // Look for JSON editor (could be textarea or a more complex editor)
        const jsonEditor = await page.$('textarea[class*="json"], [data-testid="json-editor"], .json-editor, textarea:last-of-type');
        ...
        if (jsonEditor) {
          ...
          console.log('JSON editor not found - may not be present on this page');
        }
      } catch (error) {
        console.log('Could not test JSON editor - form not accessible');
      }
    });
    ...
    it('should handle cancel action when form is accessible', async () => {
      ...
      try {
        ...
        if (cancelButton) {
          ...
          console.log(`Navigated from new lesson page to: ${newUrl}`);
        }
      } catch (error) {
  ...
  describe('Form Validation', () => {
    it('should validate slug format when form is accessible', async () => {
      ...
      const currentUrl = page.url();
      if (currentUrl.includes('/signin')) {
        console.log('Redirected to signin - skipping slug validation test');
        return;
      }
      ...
      try {
        ...
        console.log(`Found slug error element: ${slugError ? 'yes' : 'no'}`);
      } catch (error) {
        console.log('Could not test slug validation - form not accessible');
      }
    });
    ...
    it('should handle type selection when form is accessible', async () => {
      ...
      const currentUrl = page.url();
      if (currentUrl.includes('/signin')) {
        console.log('Redirected to signin - skipping type selection test');
        return;
      }
      ...
      try {
        ...
        console.log('Could not test type selection - form not accessible');
      }
    });
  });
