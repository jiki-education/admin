...
export default function LevelDetail() {
  ...
  const handleBulkEditSave = useCallback(
    async (updates: Partial<AdminLesson>) => {
      try {
        // Update each selected lesson
        const updatePromises = selectedLessonIds.map((lessonId) => updateLesson(levelId, lessonId, updates));
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div className="space-y-6">
              <div>
                ...
                <LessonTable lessons={[]} loading={true} onReorder={async () => {}} levelId={levelId} />
              </div>
            </div>
          </div>
        </div>
  ...
  if (error) {
    return (
      <div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">Level Details</h1>
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">Level Details (ID: {levelId})</h1>
            <Button variant="outline" onClick={handleBack}>
              Back to Levels
            </Button>
          </div>
          ...
          <div className="space-y-6">
            <LessonErrorBoundary>
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-medium text-gray-800 dark:text-white/90">Lessons ({lessons.length})</h2>
                  <Button onClick={handleAddNewLesson}>Add New Lesson</Button>
                </div>

                {lessons.length > 0 && (
                  <div className="mb-6">
                    <LessonFilters lessons={lessons} onFilterChange={handleFilterChange} />
                  </div>
                )}
