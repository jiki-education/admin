...
import { createCodeScene } from "@/lib/api/code-scenes";
import type { CreateCodeSceneData } from "@/lib/types/code-scenes";
import type { SceneConfig } from "@/lib/remotion/types";
import Button from "@/components/ui/button/Button";
import SceneEditor from "../components/SceneEditor";

export default function NewCodeScenePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data: CreateCodeSceneData = {
        title: title.trim(),
        description: description.trim() || undefined,
        config
      };

      const response = await createCodeScene(data);
      router.push(`/dashboard/code-scenes/${response.scene.id}`);
    } catch (err) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Create Code Scene</h1>
          <p className="text-gray-600 mt-2">Build an animated code tutorial</p>
        </div>
